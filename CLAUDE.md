...
|---|---|---|---|---|
| 1 | Auth/permission gates, 404/403 polish, deep linking, share URLs, brand-picker URL nav, suspense boundaries on `/b/:slug/design/:designSlug` | Step 9 commit 3a forward-pulled the route from Phase 4.5 to unblock the brandkit migration | **Phase 4.5** | **Mostly closed (2026-05-04 in Phase 4.5).** Inline 404/403 NotFoundPanel replaces redirect+toast; brand-picker URL nav navigates to `/b/:newSlug/design`; Share button on editor topbar copies the canonical URL. Still open: real RBAC + suspense boundaries beyond PageSpinner. |
