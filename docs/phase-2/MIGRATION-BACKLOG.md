...
| B18 | **Logo durable Asset records (the one open Brand-System blocker).** `logoSystem` refs persist via the identity blob, but `brandAssets[]` records have no DB column, so ids are re-derived from URL hashes on read — "assetId is the durable ref" is not yet true. Needs an additive `brands.brand_assets` JSONB column (migration 014, owner-deploy) OR an `AssetRepository` over the existing `public.assets` table. Then: stop hash-deriving ids, mint stable ids once, make `migrateBrandToCurrent` prefer persisted records, and hydrate logos in the flip. | brands.supabase.ts, migrateSchema.ts, assetOperations.ts, (new) AssetRepository | logo/asset vertical |
