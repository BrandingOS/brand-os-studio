...
| B16 | ~~**Setup strategy → full canonical authority.**~~ **CLOSED 2026-08-09 (authority flip).** Setup routes strategy (+about), colors, fonts, and tone through the canonical ops; `migrateBrandToCurrent.hydrateFromCanonicalIdentity` makes the identity blob the read authority for all migrated subsystems and projects `guidelines.strategy` from it on read. StrategyTab writes the blob authoritatively (no more direct `guidelines` write). | setup.tsx, migrateSchema.ts, BrandSettingsDialog.tsx | **CLOSED** |
| B18 | ~~**Logo durable Asset records.**~~ **CODE-COMPLETE 2026-08-09; authed activates on migration 014 deploy.** Added additive `brands.brand_assets` + `brands.logo_system` columns (014) + `SupabaseBrandsService` persist/read (pre-014-tolerant via `42703` retry). All current logo writers (LogosPanel, Logo Maker, asset uploads, Setup `buildLogoPatch`) stage via `stageLogoAssignment`; `migrateBrandToCurrent` prefers persisted refs/records over URL-hash re-derivation. Guest live now. **Remaining:** deploy 014 (`docs/phase-2/DEPLOY-014-runbook.md`). Follow-on (not blocking): migrate onboarding-v4 create to stage logos (currently bootstrap URLs); the Setup clear-all-logos edge doesn't clear persisted refs. | brands.supabase.ts, migrateSchema.ts, mockBrandToPatch.ts, LogosPanel.tsx | **CODE-COMPLETE** |
