...
1. Every non-deleted workspace has **≥ 1 active owner** — trigger `workspace_members_guard_last_owner` (BEFORE UPDATE/DELETE) takes `pg_advisory_xact_lock(hashtext('ws-owner:'||workspace_id))` first, then counts, and raises `last_owner` when the row being demoted/removed/suspended is the last active owner. The lock serialises two concurrent demotions of different owner rows; without it both would pass under READ COMMITTED.
...
3. A user cannot change their own role (trigger: `auth.uid() = OLD.user_id AND NEW.role <> OLD.role` → `self_role_change`), except leaving (`leave_workspace()` RPC, which applies rule 1) and `transfer_ownership`'s own self-demote step, which sets the transaction-local GUC `app.bypass_self_role_change` the trigger honours. Role changes go through `set_member_role(ws, user, role, mode, default_role)`, which writes `role` and `brand_access_mode` in one UPDATE (the CHECK constraint is evaluated per statement) and deletes the member's `brand_access` rows when promoting to owner/admin, so no second source of truth is left behind.
4. Deleting a user account: `prepare_account_purge` (existing, migration 029) is extended — for each workspace where the user is the last owner: if the workspace has other active **admins or members**, the earliest admin (else the earliest member) is promoted to owner and an audit event is written; a workspace whose only other members are **guests** is treated as having none — guests were scoped deliberately and never inherit a workspace — and is soft-deleted with the account (its guests lose access, its brands are purged with it after 30 days). No workspace is ever orphaned, and no guest ever becomes an owner.
