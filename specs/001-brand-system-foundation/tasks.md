...
## Phase 7b: Database blockers — REQUIRED before 016–021 reach production

All three are **pre-existing defects**, found while validating this release
against a real local Postgres. None is caused by this feature; all gate its
deployment.
Full detail in `docs/phase-2/DEPLOY-016-021-runbook.md`.

- [x] T087 Repair or supersede the three legacy migrations that make a fresh chain fail, so `supabase db reset` completes cleanly from 001 → latest: `20250905210043` and `20250905210159` (demo-brand INSERTs whose `user_id` subquery is NULL on an empty `auth.users`) and `20250905213158` (`CREATE POLICY … FOR SELECT … WITH CHECK`, rejected by Postgres). **Done when** a clean machine can run `supabase db reset` with zero errors — until then a deploy cannot be rehearsed against a faithful copy of production. — **DONE.** Re-run on 2026-08-13 with the complete chain present: a completely clean `supabase db reset` applies **35 migrations, 001 → 021 (`20260813050000_021_close_cross_account_holes`), with zero errors**. This supersedes the earlier record, which was taken when the chain ended at 019 and therefore did not cover 020 or 021. Nine legacy files repaired; details in the runbook.
...
- [x] T089 Close the two cross-account holes found in the FINAL policy set (not in an isolated historical file): `notifications_insert` used `WITH CHECK (true)`, letting any authenticated user create a titled, linked notification addressed to **any other user** (spam, and phishing inside a trusted surface); and `scratch_select_own` / `scratch_delete_own` were named for ownership but predicated only on `bucket_id`, so any authenticated user could read or **delete** another user's onboarding uploads. **Done when** a clean database shows a self-scoped notification insert policy and owner-scoped scratch policies, and the RLS script proves both at runtime. — **DONE.** Migration `021_close_cross_account_holes` plus `supabase/tests/021_cross_account_holes.test.sql` (✓ ALL 021 RLS ASSERTIONS PASSED). Verified beforehand that no product code calls `INotificationsService.create`, so tightening the insert policy breaks nothing. Both defects are pre-existing (001-era and 007-era); they are fixed here rather than deferred because a confirmed cross-account hole is a release blocker.
