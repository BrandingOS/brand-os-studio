...
export class HistoryRing<T> {
   ...
   *
   * This is also "what undo would reverse" — the top of `past` is the state
   * the last action produced.
   */
  currentLabel(): string | undefined {
    return this.past[this.past.length - 1]?.label;
  }

  /** Label of the step redo would re-apply, for a menu item or tooltip. */
