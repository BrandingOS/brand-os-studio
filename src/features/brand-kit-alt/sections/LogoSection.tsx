 ...
 *
 * Supports a "grouped" view that collapses visually-identical variants
 * (same logo image + filter) into cards with a background picker.
 */
import * as React from 'react';
import { useState } from 'react';
import { Link } from 'react-router-dom';
import { Edit3, Download, Sparkles, LayoutGrid, List } from 'lucide-react';
...
export function LogoSection({ brand, slug }: LogoSectionProps) {
  ...
  const groups = React.useMemo(() => collapseVariants(variants), [variants]);
  ...
  const handleGroupedDownload = (variantId: string) => {
    const variant = variants.find(v => v.id === variantId);
    if (variant) setActive(variant);
  };
  ...
  return (
    <section>
      <SectionHeader
        ...
        count={collapsed ? groups.length : variants.length}
        action={
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setCollapsed(!collapsed)}
              className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-2 text-xs font-medium text-muted-foreground hover:border-primary/40 hover:text-foreground transition"
            >
              {collapsed ? (
                <>
