...
export function AssetUploadZone({ onUpload, accept = 'image/*,application/pdf', uploading = false }: AssetUploadZoneProps) {
    ...
    >
      ...
      {uploading ? (
        <>
          ...
          <p className="text-xs text-muted-foreground">Please wait while your files are being uploaded</p>
        </>
      ) : (
        <>
          ...
          <div className="relative mt-1 inline-flex items-center gap-1.5 rounded-full border border-border bg-background/40 px-2.5 py-1 text-[10px] text-muted-foreground backdrop-blur">
            ...
            Smart category detection enabled
          </div>
        </>
      )}
