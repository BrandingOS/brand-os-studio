...
} from '../pages/templates/TemplatePages';
import {
...
export function buildEditorSlides(brand: Brand): SlideData[] {
  ...
  return [
    ...
    { id: 'intro', name: 'Introduction', render: (p) => (
      <ContentPage {...p} sectionName="Brand Overview" title="Introduction">
        <div className="grid grid-cols-2 gap-6 h-full">
          <div>
            <h4 className="text-[9px] font-semibold uppercase tracking-wider opacity-30 mb-1">Mission</h4>
            <p className="text-[clamp(10px,1vw,13px)] leading-relaxed opacity-70">{strategy?.mission || `${brand.name} exists to deliver value.`}</p>
          </div>
          <div>
            <h4 className="text-[9px] font-semibold uppercase tracking-wider opacity-30 mb-1">Vision</h4>
            <p className="text-[clamp(10px,1vw,13px)] leading-relaxed opacity-70">{strategy?.vision || 'Leading our industry.'}</p>
          </div>
        </div>
      </ContentPage>
    )},
    { id: 'values', name: 'Core Values', render: (p) => (
      <ContentPageDark {...p} sectionName="Brand Overview" title="Core Values">
        <div className="grid grid-cols-3 gap-3 mt-2">
          {(strategy?.values || ['Quality', 'Innovation', 'Trust']).map((v, i) => (
            <div key={v} className="rounded-lg p-3" style={{ backgroundColor: `${brand.primaryColor}15` }}>
    ...
    { id: 'color-section', name: 'Color System', render: (p) => <SectionDivider {...p} sectionNumber="3" sectionTitle="Color System" sectionSubtitle="Primary · Secondary" /> },
