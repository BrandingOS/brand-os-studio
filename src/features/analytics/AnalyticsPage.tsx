...
interface CR { label: string; fg: string; bg: string; ratio: number; level: 'AAA'|'AA'|'Fail' }

function compute(brand: Brand | null, activityCount: number) {
  const empty = { healthScore: 0, healthBreakdown: [] as {label:string;score:number}[], completeness: 0, contrastResults: [] as CR[], viewCount: 0, viewGrowth: 0, downloadCount: 0, viewSeries: [] as {day:string;views:number}[], categoryBreakdown: [] as {category:string;count:number}[], topSearches: [] as {term:string;count:number}[], checklist: [] as {label:string;done:boolean}[] };
  ...
  const categoryBreakdown = (brand.assets ?? []).reduce<{category:string;count:number}[]>((acc, a) => { const f = acc.find((c) => c.category === a.category); if (f) f.count++; else acc.push({ category: a.category, count: 1 }); return acc; }, []);
  const seed = brand.name.length + (brand.assets?.length ?? 0);
  const viewSeries = Array.from({ length: 30 }, (_, i) => ({ day: `${i + 1}`, views: Math.max(0, 30 + ((seed * (i + 1)) % 25) + Math.round(20 * Math.sin(i / 4))) }));
  const viewCount = viewSeries.reduce((s, p) => s + p.views, 0);

  return {
    healthScore,
    healthBreakdown: [{ label: 'Identity', score: idScore }, { label: 'Strategy', score: stScore }, { label: 'Content', score: ctScore }, { label: 'Accessibility', score: axScore }],
    completeness, contrastResults, viewCount, viewGrowth: 12 + (seed % 18),
    downloadCount: Math.round(viewCount * 0.18), viewSeries, categoryBreakdown,
    topSearches: [{ term: 'logo', count: 48 + (seed % 20) }, { term: 'color palette', count: 32 + (seed % 15) }, { term: 'business card', count: 21 + (seed % 12) }, { term: 'brand guide', count: 17 + (seed % 10) }, { term: 'social', count: 14 + (seed % 8) }],
    checklist,
  };
}
