...
/**
 * DS v1 showcase — every component of the BrandingOS design system in both
 * modes. Dev-only; the light/dark switch flips a local data-theme scope.
 */
...
export default function DesignSystemShowcasePage() {
  ...
  const palette = theme === 'light' ? dsLight : dsDark;
      ...
      >
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <DsEyebrow>BrandingOS · Design System · V1</DsEyebrow>
            <h1
              style={{
                margin: 0,
                fontWeight: 800,
        ...
        <Section title="Color tokens">
          <Row>
            {Object.entries(palette).map(([name, hex]) => (
              <div key={name} style={{ display: 'flex', flexDirection: 'column', gap: 6, width: 92 }}>
                <div
                  style={{
                    height: 36,
                    borderRadius: 10,
                ...
                />
                <span style={{ fontSize: 11, color: 'var(--ds-text-secondary)' }}>{name}</span>
                <span className="ds-mono" style={{ fontSize: 10 }}>{hex}</span>
              </div>
            ))}
          </Row>
        </Section>
        ...
        <DsConfirmDialog
          ...
          description={
            <>
              "Wordmark · Original" will be removed from the kit and every generated asset that
